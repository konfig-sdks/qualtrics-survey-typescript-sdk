...
import { readableStreamToString, QualtricsError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new QualtricsError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new QualtricsError(e, e.response?.data)
                }
                if (innerError instanceof QualtricsError) {
                    // Got "QualtricsError" from the above try block
                    throw innerError;
                }
